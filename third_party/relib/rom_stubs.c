/* Stubs to improve Ghidra function detection
 * These are convenient because they are loaded as part of the elf file.
 * However a proper ghidra data type library might be better.
 */
...
{
	return CALL3(0x4000dea8, s1, s2, n);
}

void*
memcpy(void *dest, const void *src, size_t n)
{
	return (void*)CALL3(0x4000df48, dest, src, n);
}

void
*memset(void *s, int c, size_t n)
{
	return (void*)CALL3(0x4000e190, s, c, n);
}

c_exception_handler_t _xtos_set_exception_handler(int cause, c_exception_handler_t fn)
{
	return (c_exception_handler_t)CALL2(0x40000454, cause, fn);
}

uint32_t
rtc_get_reset_reason(void)
{
	return CALL0(0x400025e0);
}
